...
class Transaction(Base):
    ...
    # The deterministic ordering key `GET /api/transactions` sorts by — the
    # same reasoning `AuditEvent.sequence` already documents: a UUID primary
    # key has no order of its own, and `created_at` (`server_default=func.now()`)
    # is constant for every row inserted in the same DB transaction (e.g.
    # several transactions created back-to-back in one request or one test),
    # so it can't be relied on alone either. Database-generated
    # (`GENERATED BY DEFAULT AS IDENTITY`), so it's correct under concurrent
    # writers without any application-computed counter.
